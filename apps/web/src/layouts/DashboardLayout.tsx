...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Policy Overview', path: '/', icon: LayoutDashboard },
    { name: 'Policy Library', path: '/library', icon: FileCode },
    { name: 'Compliance Matrix', path: '/compliance', icon: ClipboardList },
    { name: 'Evaluation Logs', path: '/evaluation', icon: Terminal },
    { name: 'Governance Center', path: '/governance', icon: ShieldCheck },
    { name: 'Security Baselines', path: '/security', icon: Lock },
    { name: 'Cloud Policies', path: '/cloud', icon: Globe },
    { name: 'K8s Policies', path: '/kubernetes', icon: Database },
    { name: 'CI/CD Guardrails', path: '/cicd', icon: Zap },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-emerald-400 bg-clip-text text-transparent">POLILIB</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">Policy Architect</p>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search policies, compliance frameworks, or violation reports..." 
